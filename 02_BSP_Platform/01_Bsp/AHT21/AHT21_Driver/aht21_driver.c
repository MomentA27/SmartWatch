...
static uint8_t aht21_read_status(bsp_aht21_driver_t * const p_aht21)
{
  ...
  uint8_t rx_data = 0;
  /**********************************临界区*************************************/
  // ✨【修改层级】通过 p_driver_api 访问
  p_aht21->p_driver_api->p_i2c_driver->pf_critical_enter();
  {
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_start(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_READ_ADDR);
    uint8_t ret = p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    ASSERT_CONDITION(ret == AHT21_OK);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL,&rx_data);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_nack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_stop(NULL);
  }
  /********************************临界区结束************************************/
...
static aht21_status_t aht21_init(bsp_aht21_driver_t * const p_aht21)
{
  ...
  ASSERT_NOT_NULL(p_aht21->p_driver_api);
  ASSERT_NOT_NULL(p_aht21->p_driver_api->p_i2c_driver);
  ASSERT_NOT_NULL(p_aht21->p_driver_api->p_i2c_driver->pf_i2c_init);
  if (IS_INITED) return AHT21_ERRORRESOURCE;
  p_aht21->p_driver_api->p_yield->pf_rtos_yield(300);

  p_aht21->p_driver_api->p_i2c_driver->pf_i2c_init(NULL);
                             ...
                             uint8_t             * const state_num)
{
  ...
  p_aht21->p_driver_api->p_i2c_driver->pf_critical_enter();
  {
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_start(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_READ_ADDR);
    aht21_status_t ret = p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    ASSERT_CONDITION(ret == AHT21_OK);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL, state_num);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_nack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_stop(NULL);
  }
  /********************************临界区结束*************************************/
                                           ...
                                           float              * const humi   )
{
  ...
  {
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_start(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_WRITE_ADDR);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_MEASURE_CMD);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_MEASURE_CMD_ARFS1);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL, AHT21_REG_MEASURE_CMD_ARFS2);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_wait_ack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_stop(NULL);
  }
  p_aht21->p_driver_api->p_i2c_driver->pf_critical_exit();

  p_aht21->p_driver_api->p_yield->pf_rtos_yield(AHT21_MEASURE_WAITING_TIME);
  /********************************临界区结束*************************************/
  // 2.等待测温完成
  p_aht21->p_driver_api->p_yield->pf_rtos_yield(AHT21_MEASURE_WAITING_TIME);
  ...
  {
    p_aht21->p_driver_api->p_yield->pf_rtos_yield(5);
    cnt--;
  ...
  p_aht21->p_driver_api->p_i2c_driver->pf_critical_enter();
  {
    ...
    for (uint8_t i = 0; i < 5; i++) {
      p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL, &data_buf[i]);
      p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_ack(NULL);
    }
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL, &data_buf[5]);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_nack(NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_stop(NULL);
  }
  /********************************临界区结束*************************************/
                                      ...
                                      float              * const humi   )
{
  ...
  {
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_start    (NULL);
    p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_byte(NULL,
    ...
    p_aht21->p_driver_api->p_i2c_driver->pf_critical_enter();
  {
    ...
    {
      p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL,
                                                                      &data_buf[i]);
      p_aht21->p_driver_api->p_i2c_driver->pf_i2c_send_ack    (NULL);
    }
      p_aht21->p_driver_api->p_i2c_driver->pf_i2c_receive_byte(NULL,
...
aht21_status_t aht21_inst(
    ...
    aht21_driver_input_api_t* const p_driver_api)
{
  ASSERT_NOT_NULL(p_bsp_aht21_inst);
  ASSERT_NOT_NULL(p_driver_api);
  // 统一注入依赖包 通过结构体封装整个driver层需要的接口
  p_bsp_aht21_inst->p_driver_api = p_driver_api;
  //绑定方法实现
